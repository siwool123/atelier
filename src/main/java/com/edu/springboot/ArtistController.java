...
public class ArtistController {
	...
	//경매작품등록
	@PostMapping("artist/addAuction.do")
	public String addAuctionProcess(Principal principal, Model model, ProductDTO pdto, HttpServletRequest req) {
		...
		try {
			...
			int result = ardao.auctioninsert(pdto);
			if (result == 1) {
				System.out.println("상품 업로드 성공");
			} else {
				System.out.println("상품 업로드 실패");
			...
			e.printStackTrace();
			return "redirect:/artist/addAuction";
		}
		return "redirect:/artist/addAuction";
	}
